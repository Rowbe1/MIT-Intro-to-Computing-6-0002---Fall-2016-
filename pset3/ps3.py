...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        dirt = self.room[(tile_x,tile_y)]
        if capacity > dirt:
            cleaned = 0
        else:
            cleaned = dirt - capacity
        self.room[(tile_x,tile_y)] = cleaned
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        # rand_x = random.randint(0,room.width - 1)
        # rand_y = random.randint(0,room.height - 1)
        # self.position = Position(rand_x,rand_y)
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        tile_x = math.floor(pos_x)
        tile_y = math.floor(pos_y)
        return (tile_x, tile_y) in self.furniture_tiles
    ...
    def is_position_valid(self, pos):
        ...
        """
        unfurnished = not self.is_position_furnished(pos)
        in_room = self.is_position_in_room(pos)
        return in_room and unfurnished
    ...
    def get_num_tiles(self):
        ...
        """
        self.num_tiles = self.width * self.height
        self.avail_tiles = self.num_tiles - len(self.furniture_tiles)
        return self.avail_tiles
    ...
    def get_random_position(self):
        ...
        self.rand_pos = random_pos()

        while self.is_position_furnished(self.rand_pos) == True:
            self.rand_pos = random_pos()
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        self.new_pos = self.position.get_new_position(self.direction,self.speed)
        if self.room.is_position_valid(self.new_pos):
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    def clean_trial(num_robots, width, height, clean_fraction, min_coverage):
        #anim = ps3_visualize.RobotVisualization(num_robots, width, height,False, 0.01)
        count = 0
        robots = []
        ...
        for i in range(num_robots):
            ...
            robots.append(robot)
        while clean_fraction < min_coverage:
            for bot in robots:
                bot.update_position_and_clean()
            clean_fraction = room.get_num_cleaned_tiles() / room.get_num_tiles()
            count += 1
    ...
    for i in range(num_trials):
        times.append(clean_trial(num_robots, width, height, clean_fraction, min_coverage))
    #print("TIMES",times)
