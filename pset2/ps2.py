    ...
    for edge in digraph.get_edges_for_node(start):
        ...
        if dest not in tmp_path: #avoid cycles
            if (best_path == None or (tmp_length + total) < best_dist) and (tmp_outdoor + outdoor) <= max_dist_outdoors:
                newPath = get_best_path(digraph, dest, end, [tmp_path, tmp_length + total, tmp_outdoor + outdoor], 
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    if get_best_path(digraph, start, end, [[],0,0], max_dist_outdoors, 0, None)[0] == None or\
        get_best_path(digraph, start, end, [[],0,0], max_dist_outdoors, 0, None)[1] > max_total_dist:
            raise ValueError("No suitable path")
    else:
        return get_best_path(digraph, start, end, [[],0,0], max_dist_outdoors, 0, None)[0]
